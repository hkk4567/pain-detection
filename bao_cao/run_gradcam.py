...
# ================================================================
# 1. ĐIỀN TÊN THƯ MỤC CHỨA CHECKPOINT VÀ ẢNH MUỐN XEM
# ================================================================
# 2026-04-15_11-21-45_GRAY_PureRaw_Baseline_fold_1.pth
...
# ================================================================
# GIỮ NGUYÊN CÁC CONFIG & MODEL CỦA BẠN
# ================================================================
class Config:
...
# ================================================================
# HÀM UN-NORMALIZE ĐỂ VẼ ẢNH GỐC
# ================================================================
def unnormalize_and_to_rgb(tensor_img):
    """Đảo ngược Normalize để lấy lại ảnh gốc phục vụ vẽ Heatmap"""
    mean = np.array([0.449, 0.449, 0.449]) # Dùng mean/std của GRAY model (Sửa nếu test model RGB)
    std = np.array([0.226, 0.226, 0.226])
...
# ================================================================
def main():
    print("🚀 BẮT ĐẦU CHẠY ENSEMBLE GRAD-CAM...")
    ...
    # --- CHỈNH SỬA: Tìm index của ảnh mục tiêu ---
    target_idx = -1
    ...
    for i, idx in enumerate(indices_to_show):
        ...
        save_path = os.path.join(export_dir, f"ensemble_cam_{img_name}.png")
        plt.savefig(save_path, dpi=200, bbox_inches='tight')
        plt.close()
        print(f"  -> Đã lưu {save_path}")

    print(f"\n🎉 HOÀN TẤT! Ảnh ENSEMBLE Grad-CAM đã được lưu vào thư mục:\n{export_dir}")
